    ...
    def write(
        ...
        Args:
            ...
            include_files:  If True, embed the run directories into the crate.
                            If False, reference them as logical run IDs and artifact hashes.
        """
        crate = ROCrate()
        crate.name = "Workflow Run Comparison"
        crate.description = (
            f"Comparison of run1 vs run2 - "
            f"{summary.get('verdict', 'PASS' if summary['overall_match'] else 'FAIL')}"
        )
        ...
        if include_files:
            ...
            run1_entity = ContextEntity(
                crate,
                identifier='#run1',
                properties={'@type': 'Dataset', 'name': 'Run 1', 'description': 'External inputs; identify using the report artifact hashes.'}
            )
            crate.add(run1_entity)
            run2_entity = ContextEntity(
                crate,
                identifier='#run2',
                properties={'@type': 'Dataset', 'name': 'Run 2', 'description': 'External inputs; identify using the report artifact hashes.'}
            )
        ...
        packaged = deepcopy(summary)
        for side, run in packaged.get('metadata', {}).get('runs', {}).items():
            run['storage'] = 'embedded' if include_files else 'external'
            run['root'] = side + '/' if include_files else None
        result_json = json.dumps(packaged, indent=2, cls=NumpyEncoder, allow_nan=False)
        ...
        try:
            ...
            summary_entity = ContextEntity(
                ...
                properties={
                    ...
                    'value': summary.get('verdict', 'PASS' if summary['overall_match'] else 'FAIL'),
                    'description': (
            ...
            crate.root_dataset['mentions'] = [{'@id': summary_entity.id}, {'@id': run1_entity.id}, {'@id': run2_entity.id}]
